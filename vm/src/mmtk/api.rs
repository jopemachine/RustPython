...
use mmtk::Mutator;
use crate::mmtk::DummyVM;
use crate::mmtk::SINGLETON;
use crate::mmtk::BUILDER;
...
pub extern "C" fn mmtk_init(heap_size: usize) {
    ...
    assert!(!crate::mmtk::MMTK_INITIALIZED.load(Ordering::SeqCst));
...
#[no_mangle]
pub extern "C" fn mmtk_process(name: &CStr, value: &CStr) -> bool {
    let mut builder = BUILDER.lock().unwrap();
    memory_manager::process(&mut builder, name.to_str().unwrap(), value.to_str().unwrap())
}
